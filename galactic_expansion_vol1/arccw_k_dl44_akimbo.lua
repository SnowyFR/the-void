...
SWEP.WorldModel = "models/arccw/kraken/galactic/worldmodels/w_dl44.mdl"
...
SWEP.ShootSound = "ArcCW_Kraken.SW_GALACTIC_DL44_Close"
SWEP.DistantShootSound = "ArcCW_Kraken.SW_GALACTIC_DL44_Distant"
...
if CLIENT then
    SWEP.HandWM = {
        RH = { -- Right Hand
            model = "models/arccw/kraken/galactic/worldmodels/w_dl44.mdl",
            att   = "anim_attachment_RH",
            bone  = "ValveBiped.Bip01_R_Hand",
            pos   = Vector(1,  8,  3),  
            ang   = Angle(0,   0,  0), 
            scale = 1,
            Bodygroups = "001000000000"
        },
        LH = { -- Left Hand
            model = "models/arccw/kraken/galactic/worldmodels/w_dl44.mdl",
    ...
    local function buildWM(wep, key)
        ...
        -- Aplicar bodygroups personalizados definidos en HandWM.
        if cfg.Bodygroups then
            if isstring(cfg.Bodygroups) then
                -- Cada carácter numérico representa el bodygroup para ese índice.
                for i = 0, #cfg.Bodygroups - 1 do
                    local c = string.sub(cfg.Bodygroups, i + 1, i + 1)
                    local num = tonumber(c)
                    if num then
                        m:SetBodygroup(i, num)
                    end
                end
            elseif istable(cfg.Bodygroups) then
                for bg, val in pairs(cfg.Bodygroups) do
                    if isnumber(bg) and isnumber(val) then
                        m:SetBodygroup(bg, val)
                    end
                end
            end
        end
